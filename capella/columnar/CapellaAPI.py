...
class CapellaAPI(CommonCapellaAPI):
    ...
    def get_dp_agent_hash(self, cluster_id):
        ...
        resp = self.do_internal_request(url, method="GET", headers=self.cbc_api_request_headers)
        return resp
    ...
    def update_dp_agent(self, cluster_id, hash):
        ...
        url = '{}/internal/support/clusters/{}/agent-versions/activate' \
            .format(self.internal_url, cluster_id)
        resp = self.do_internal_request(url, method="POST", params=json.dumps({"hash": hash}), headers=self.cbc_api_request_headers)
        return resp